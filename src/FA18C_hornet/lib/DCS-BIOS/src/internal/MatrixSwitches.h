...
public:
  Matrix2PosT(const char *msg, char row, char col, bool reverse)
      : PollingInput(pollIntervalMs) {
    init_(msg, row, col, reverse);
  }

  Matrix2PosT(const char *msg, char row, char col)
      : PollingInput(pollIntervalMs) {
...
private:
  ...
  char lastState_;
  char readState() {
    if (swPanel.GetSwitchState(rowA_, colA_) == true) {
      return 0;
    }
    if (swPanel.GetSwitchState(rowB_, colB_) == true)
      return 2;
    return 1;
  }
  void resetState() { lastState_ = (lastState_ == 0) ? -1 : 0; }
...
public:
  Matrix3PosT(const char *msg, char rowA, char colA, char rowB, char colB)
      : PollingInput(pollIntervalMs) {
    msg_ = msg;
    colA_ = colA;
    rowA_ = rowA;
