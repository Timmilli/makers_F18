 ...
 * The return type SwitchState is an enumerated type, defined in the header
 * file. It can have four values: FALSE, TRUE, DEBOUNCE, NONSET. This actually
 * extends the standard bool type, and can be compared and converted to logical
 * bool.
 *
 * *****	void GetAllSwitchStates(SwitchState
 * (&arrayToFill)[numberOfRows][numberOfCols]);
 *
 * This functions takes an array thats bounds match the number of rows and the
 * number of coloumns of the matrix by reference, and fills it with the state of
 * each switches in the matrix.
 *
 * ***** void AddEventListener(void (*listener)(OWNSTATUS)):
 *
 * This function sets an event listener which is being called by the Scan()
 * function in case a switch has been turned on or off. The function has to take
 * one OWNSTATUS type parameter. OWNSTATUS is a struct defined in the header file. it
 * contains two byte type values: row, col, which are the coloumn and row of the
 * switches that have been flipped, and a SwitchState type which is the new
 * state of the switch
 *
 * description of file:
 *
...
// And we need to overload the == operator to be able to compare it to a logic
// variable
...
// This struct is used when returning the status of an actual switch on user
// request
typedef struct {
  byte row;
  byte col;
...
private:
  ...
  void (*eventListener)(OWNSTATUS);

public:
  SwitchMatrix();
  void Scan();
  SwitchState GetSwitchState(int row, int col);
  void GetAllSwitchStates(
      SwitchState (&arrayToFill)[numberOfRows][numberOfCols]);
  void AddEventListener(void (*listener)(OWNSTATUS));
};
