...
public:
  virtual void onDcsBiosWrite(unsigned int address __attribute__((unused)),
  ...
  ExportStreamListener(uint16_t firstAddressOfInterest,
                       uint16_t lastAddressOfInterest) {
    ...
    // keep list ordered ascending by lastAddressOfInterest, then ascending
    // firstAddressOfInterest
    ExportStreamListener **prevNextPtr = &firstExportStreamListener;
    ExportStreamListener *nextESL = firstExportStreamListener;

    while (
        nextESL &&
        ((nextESL->getLastAddressOfInterest() < this->lastAddressOfInterest) ||
         (nextESL->getLastAddressOfInterest() == this->lastAddressOfInterest &&
          nextESL->getFirstAddressOfInterest() <
              this->firstAddressOfInterest))) {
...
private:
  ...
  enum FLAGS { DIRTY = (1 << 0), RECEIVED_DATA_BEFORE = (1 << 2) };

public:
  Int16Buffer(unsigned int address)
      : ExportStreamListener(address, address), flags(0) {}
  virtual void onDcsBiosWrite(unsigned int address __attribute__((unused)),
                              unsigned int data) {
    if (this->flags == 0) {
      // first update ever
...
private:
  ...
  void setChar(unsigned int index, unsigned char value) {
    ...
    if (receivingBuffer[index] == value)
      return;
...
public:
  StringBuffer(unsigned int address, void (*callback)(char *))
      : ExportStreamListener(address, address + LENGTH) {
