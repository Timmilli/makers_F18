...
template <unsigned long pollIntervalMs = POLL_EVERY_TIME,
          unsigned int hysteresis = 128, unsigned int ewma_divisor = 5>
...
private:
  ...
  void pollInput() {
    unsigned int state;
    if (reverse_)
      state = map(readState(), input_min_, input_max_, 65535, 0);
    else
      state = map(readState(), input_min_, input_max_, 0, 65535);
...
public:
  PotentiometerEWMA() : PollingInput(pollIntervalMs) {}

  PotentiometerEWMA(const char *msg, bool reverse = false,
                    unsigned int input_min = 0, unsigned int input_max = 1023)
    ...
    if (reverse_)
      lastState_ = map(readState(), input_min_, input_max_, 65535, 0);
    else
      lastState_ = map(readState(), input_min_, input_max_, 0, 65535);
  }

  virtual unsigned int readState();

  void setAttributes(const char *msg, unsigned int lastState, bool reverse,
                     unsigned int input_min, unsigned int input_max) {
    msg_ = msg;
